...
class SKGravityShortcodes
{
	...
	public function sk_gravity_inpage($atts, $content = null){
		...
		for ($i=0; $i < $sectionsNo; $i++){
			...
			//section
			$out .= '<div id="'.$gravity_engine->getStaticID($i).'" data-isfullheight="'.$gravity_engine->_eIsSectionFullHeight($i).'" data-scrollduration="'.$gravity_engine->getSectionScrollDuration($i).'" class="gravitySection" data-hasbackgroundimage="'.$backSettings['hasBackground'].'" data-backimageurl="'.$backSettings['backImageSrc'].'" style="'.$gravity_engine->getSectionCSS($i).'">';
					//top arrow
